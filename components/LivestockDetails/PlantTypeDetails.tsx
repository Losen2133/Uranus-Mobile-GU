...
import { capitalize, convertTemperature, formatDate, getDaysLeft, getTemperatureSuffix, TempUnit } from "@/utils/stringUtils";
...
type PlantTypeDetailsProps = {
    livestock: LivestockData | null;
    userSettings: UserSettings | null;
};

export function PlantTypeDetails({ livestock, userSettings }: PlantTypeDetailsProps) {
    const tempUnit: TempUnit = userSettings?.temp_unit as TempUnit || "celcius";

    return (
        livestock?.type === "plant" && (
            <VStack
                space="md"
                className="mb-3"
            >
                ...
                {!livestock.harvested && (
                    <>
                        ...
                        <VStack space="xs">
                            ...
                            <Text className="text-center">
                                {capitalize(livestock.data.phase)}
                            </Text>
                        </VStack>
                    </>
                )}
                ...
                {livestock.data.phase === "nursery" && !livestock.harvested ? (
                    <VStack space="xs">
                        <Text className="font-bold text-lg text-center">
                            Nursery Days
                        </Text>
                ...
                ) : livestock.data.phase === "growbed" && !livestock.harvested ? (
                    <VStack space="xs">
                        <Text className="font-bold text-lg text-center">
                            Growbed Days
                        </Text>
                        ...
                        <Text className="text-center">
                            {getDaysLeft(
                                livestock.data.phase_changed_on,
                                livestock.data.harvest_days
                            ) !== 0
                                ? `${getDaysLeft(
                                    livestock.data.phase_changed_on,
                                    livestock.data.harvest_days
